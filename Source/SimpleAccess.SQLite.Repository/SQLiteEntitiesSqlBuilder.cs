...
namespace SimpleAccess.SQLite
{
    ...
    public class SQLiteEntitiesSqlBuilder : SQLiteSqlBuilder, ISqlBuilder<SQLiteParameter>
    {
        ...
        public string CreateInsertStatement()
        {
            ...
            {
                sqliteParameter = parameterInfo.Value;
                ...
                if (primaryKeyAttribute == null)
                {
                    ...
                    keyProperty = sqliteParameter.ParameterName;
                    if (primaryKeyAttribute.IsIdentity) continue;
                }


 
                columns.Add($"[{sqliteParameter.ParameterName.Substring(1)}]");
                parameters.Add(sqliteParameter.ParameterName.Substring(1));
            }
            ...
            {
                query = $"INSERT INTO {EntityInfo.DbObjectName} \n\t({String.Join("\n\t, ", columns)})\n\t VALUES \n\t(@{String.Join("\n\t, @", parameters)}); || SELECT last_insert_rowid();";
            }
            else if (primaryKeyAttribute.DbSequence != null)
            {
                throw new NotSupportedException("Sequence are not supported in SQLite.");
        ...
        public string CreateUpdateStatement()
        {
            ...
            {
                SQLiteParameter = parameterInfo.Value;
                var columnName = SQLiteParameter.ParameterName.Substring(1);
        ...
        {
            return $"DELETE FROM {EntityInfo.DbObjectName}";
        }
