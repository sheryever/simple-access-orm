...
namespace SimpleAccess.Core
{
    ...
    public static class DataReaderToObjectExtensions
    {
            ...
            , Dictionary<string, PropertyInfo> piList = null, Dictionary<string, PropertyInfo> piListBasedOnDbColumn = null)
        {
            ...
            TypeInfo propertyTypeInfo = null;
                ...
                if (piList.ContainsKey(name))
                {
                    ...
                    if ((prop != null) && prop.CanWrite)
                    {
                        try
                        {
                            ...
                            propertyTypeInfo = prop.PropertyType.GetTypeInfo();
                            if (propertyTypeInfo.IsGenericType)
                            {
                                if (propertyTypeInfo.GetGenericArguments()[0].GetTypeInfo().IsEnum)
                                {
                                    prop.SetValue(instance, (val == DBNull.Value)
                                        ? null
                                        : Enum.Parse(propertyTypeInfo.GetGenericArguments()[0],
                                            val.ToString())
                                        , null);
                                    continue;
                                }
