...
namespace SimpleAccess.SQLite.Test
{
     public class SimpleAccessTest
    {
        ...
        {
            var categoriesCount = SimpleAccess.ExecuteScalar<long>("Select Count(*) FROM Categories");
        ...
        {
            var categoriesCount = SimpleAccess.ExecuteScalar<long>("Select Count(*) FROM Categories WHERE Id > @id", new { Id = 0 });
        ...
        {
            var categoriesCount = SimpleAccess.ExecuteScalar<long>("Select Count(*) FROM Categories WHERE Id > @id"
        ...
        {
            var categoriesCount = SimpleAccess.ExecuteScalar<long>("Select Count(*) FROM Categories WHERE Id > @id", 0.ToDataParam("Id"));
        ...
        public void ExecuteScalarWithTransactionTest()
        {
            ...
            {
                var rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM Categories");

                Assert.Equal(3, rowCount);

                rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM [Branches]");

                Assert.Equal(3, rowCount);

                rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM [Attachments]");
                ...
                try
                {
                    var a = new
                        Attachment2
                        {OtherName = "OtherName" };
                    rowCount = SimpleAccess.ExecuteScalar<long>(transaction,
        ...
        public void ExecuteScalarWithTransactionAndObjectParamTest()
        {
            using (var transaction = SimpleAccess.BeginTransaction())
            {
                var rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM Categories WHERE Id > @id", 0.ToDataParam("Id"));

                Assert.Equal(3, rowCount);

                rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM [Branches] WHERE Id > @id", 0.ToDataParam("Id"));

                Assert.Equal(3, rowCount);

                rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM [Attachments] WHERE Id > @id", 0.ToDataParam("Id"));
        ...
        public void ExecuteScalarWithTransaction_CommandTypeAndObjectParamTest()
        {
            using (var transaction = SimpleAccess.BeginTransaction())
            {
                ...
                var rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM Categories WHERE Id > @id"
                    , CommandType.Text, new {id =0});

                Assert.Equal(3, rowCount);

                rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM [Branches] WHERE Id > @id"
                    , CommandType.Text, new { id = 0 });

                Assert.Equal(3, rowCount);

                rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM [Attachments] WHERE Id > @id"
                    , CommandType.Text, new { id = 0 });
        ...
        public void ExecuteScalarWithTransaction_CommandTypeAndParametersTest()
        {
            using (var transaction = SimpleAccess.BeginTransaction())
            {
                var rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM Categories WHERE Id > @id", CommandType.Text, 0.ToDataParam("Id"));

                Assert.Equal(3, rowCount);

                rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM [Branches] WHERE Id > @id", CommandType.Text, 0.ToDataParam("Id"));

                Assert.Equal(3, rowCount);

                rowCount = SimpleAccess.ExecuteScalar<long>(transaction, "Select Count(*) FROM [Attachments] WHERE Id > @id", CommandType.Text, 0.ToDataParam("Id"));
        ...
        {
            var category = SimpleAccess.ExecuteEntity<Category>("Select Id, Name, Description FROM Categories LIMIT 1");
        ...
        public void ExecuteEntityWithTransactionTest()
        {
            using (var transaction = SimpleAccess.BeginTransaction())
            {
                var category = SimpleAccess.ExecuteEntity<Category>(transaction, "Select Id, Name, Description FROM Categories LIMIT 1");

                Assert.NotNull(category);
                Assert.Equal(1, category.Id);

                var branch = SimpleAccess.ExecuteEntity<Branch>(transaction, "Select Id, CityId, Name, [PhoneNumbers], [Address], [Address2] FROM [Branches] LIMIT 1");

                Assert.NotNull(branch);
                Assert.Equal(1, branch.Id);

                var attachment = SimpleAccess.ExecuteEntity<Attachment>(transaction, "Select Id, [IncidentId], [OtherName] FROM [Attachments] LIMIT 1");
        ...
        {
            var category = SimpleAccess.ExecuteDynamic("Select Id, Name, Description FROM Categories LIMIT 1");
        ...
        public void ExecuteDynamicWithTransactionTest()
        {
            using (var transaction = SimpleAccess.BeginTransaction())
            {
                var category = SimpleAccess.ExecuteDynamic(transaction, "Select Id, Name, Description FROM Categories LIMIT 1");

                Assert.NotNull(category);
                Assert.Equal(category.Id, 1);

                var branch = SimpleAccess.ExecuteDynamic(transaction, "Select Id, CityId, Name, [PhoneNumbers], [Address], [Address2] FROM [Branches] LIMIT 1");

                Assert.NotNull(branch);
                Assert.Equal(branch.Id, 1);

                var attachment = SimpleAccess.ExecuteDynamic(transaction, "Select Id, [IncidentId], [OtherName] FROM [Attachments] LIMIT 1");
